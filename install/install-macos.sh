...
# install-macos.sh
# Version: 1.0.0
# macOS installer for claude-usage-on-icon (v1: SwiftBar/xbar plugin).
#
# What it does:
...
#   - Copies the SwiftBar plugin to ~/Library/Application Support/SwiftBar/Plugins/
#
# Requires SwiftBar (https://swiftbar.app) or xbar (https://xbarapp.com) to be installed.
...
PLUGIN_NAME="claude-usage.15s.sh"
SWIFTBAR_PLUGINS_DIR="$HOME/Library/Application Support/SwiftBar/Plugins"
DEST_PLUGIN="$SWIFTBAR_PLUGINS_DIR/$PLUGIN_NAME"
...
usage() {
    cat <<EOF
claude-usage-on-icon macOS installer v$VERSION
...
PYEOF
    ...
    rm -f "$DEST_WRITER" "$DEST_PLUGIN"
    echo "removed writer and SwiftBar plugin (if they existed)"
    exit 0
fi

SRC_WRITER="$(cd "$SCRIPT_DIR/.." && pwd)/writer/statusline.py"
SRC_PLUGIN="$(cd "$SCRIPT_DIR/.." && pwd)/tray/macos/swiftbar/$PLUGIN_NAME"
for f in "$SRC_WRITER" "$SRC_PLUGIN"; do
    if [ ! -f "$f" ]; then
...
if [ ! -d "$SWIFTBAR_PLUGINS_DIR" ] && [ "$DRY_RUN" != "1" ]; then
    echo "note: SwiftBar plugins folder not found at:" >&2
    echo "  $SWIFTBAR_PLUGINS_DIR" >&2
    echo "Install SwiftBar (https://swiftbar.app) or xbar (https://xbarapp.com) first," >&2
    echo "point it at a plugin folder, then re-run this installer." >&2
    exit 1
fi
...
chmod +x "$DEST_PLUGIN"
echo "installed SwiftBar plugin -> $DEST_PLUGIN"

echo
echo "Done. Open SwiftBar (or xbar) and refresh its plugins to see the icon in your menu bar."
